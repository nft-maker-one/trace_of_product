...
func main() {
	for {
		ip := flag.String("ip", "127.0.0.1", "please set the ip for node")
		port := flag.String("port", "8081", "please set the port for node")
		clientMode := flag.Bool("client", false, "set the node with clientMode")
		chainMode := flag.Bool("chain", false, "set the node with chainMode")
		help := flag.Bool("h", false, "note for inputs")
		netAddr := *ip + ":" + *port
		flag.Parsed()
		if *help {
